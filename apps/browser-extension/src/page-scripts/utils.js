  ...
  try {
    ...
    notifyOnErrorOccurred({
      ...
      if (!logShown) {
        logShown = true;
        console.log(
          `%cRequestai%c Please reload the page for ${ruleType} rule to take effect`,
...
/**
 * Simple utility to get absolute URL from relative URL
 */
...
/**
 * Parse JSON safely, returning default value if parsing fails
 */
...
/**
 * Execute a function from a string of code
 */
export const executeFunction = (code, args) => {
  try {
...
/**
 * Notify the extension about a request/response modification
 */
export const notifyExtension = (action, data) => {
  window.top.postMessage(
    {
      source: 'requestai:client',
      action,
