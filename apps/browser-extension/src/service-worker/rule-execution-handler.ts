...
class RuleExecutionHandler {
  ...
  processTabCachedRulesExecutions = async (tabId: number) => {
    ...
    const uniqueAppliedRuleIds = Array.from(
      new Set(rulesExecutionLogs.map(executionLog => executionLog.ruleId))
    );

    const appliedRules = (await rulesStorageService.getRules(uniqueAppliedRuleIds)).reduce(
      (acc, rule) => {
        acc[rule.id] = rule;
        return acc;
      },
