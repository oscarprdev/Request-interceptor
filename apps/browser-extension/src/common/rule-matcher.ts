...
const checkRegexMatch = (regexString: string, inputString: string): boolean => {
  ...
  return regex?.test(inputString) ?? false;
};
...
const matchRequestWithRuleSourceFilters = function (
  sourceFilters: RuleSourceFilter[] | undefined,
  requestDetails: AJAXRequestDetails
) {
  if (
    !sourceFilters ||
    !requestDetails ||
    (Array.isArray(sourceFilters) && sourceFilters?.length === 0)
  ) {
    return true;
  }
...
export const populateRedirectedUrl = (
  rulePair: RulePair,
  ruleType: RuleType,
  requestDetails: AJAXRequestDetails
) => {
